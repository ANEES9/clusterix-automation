...
const { logFilePath, logPerformancePath, expectedMetrics, weights, idealNavigationTime } = performanceConfig;
...
export function calculatePerformancePercentage(metrics: Record<string, number>, expected: Record<string, number>, weights: Record<string, number>) {
    ...
    const overallPerformance = totalWeights > 0 ? totalWeightedScore / totalWeights : 0;
...
export function calculateNavigationTimePercentage(timeTaken: number, idealTime: number) {
    return Math.max(0, (1 - (timeTaken - idealTime) / idealTime) * 100);
}

export async function logPerformanceData(testName: string, timeTaken: number, metrics?: Record<string, any>) {
    ...
    if (metrics) {
        const { metricResults, overallPerformance } = calculatePerformancePercentage(metrics, expectedMetrics, weights);
