...
export class EmployeeManagementPage {
  ...
  private expandIconLocator: Locator
  private hideEmployeeTextLocator: Locator
  private magnifierButtonLocator: Locator
  private searchContractTypeLocator: Locator
  private searchLocationLocator: Locator
  private searchDepartmentLocator: Locator
  ...
  private genderSelectionDropdownLocator: Locator
  ...
  private contractTypeSelectionDropdownLocator: Locator
  private recruiterFilterCategoryLocator: Locator
  private recruiterSelectionDropdownLocator: Locator
  ...
  private departmentSelectionDropdownLocator: Locator
  private wahyRecruiterOptionLocator: Locator
  private clxTesting: Locator
  private roleITOptionLocator: Locator
  private hrDepartmentOptionLocator: Locator
  private contractStatusFilterCategory: Locator
  private contractStatusDropdown: Locator
  private contractStatusIn2MonthsOption: Locator


  private contractStatusAllOption: Locator
  ...
  constructor(page: Page, locale: string) {
    ...
    this.expandIconLocator = page
      .locator('div')
      .filter({
        hasText: /^(\d+)\s(?:Table ItemsExport all|ElementeAlles exportieren)$/,
      }) // Matches both English and German
      .getByRole('button')
      .nth(2)


    this.hideEmployeeTextLocator = page.locator(
      '//*[@class="MoreDetailsExpandedBar-module_filterChips__vdLgW"]/div/div[1]'
    )


    this.searchContractTypeLocator = this.page.locator(
      '(//*[@class="_inner_16oey_48"])[5]'
    )
    this.searchLocationLocator = this.page.locator(
      '(//*[@class="_inner_16oey_48"])[6]'
    )
    this.searchDepartmentLocator = this.page.locator(
      '(//*[@class="_inner_16oey_48"])[8]'
    )
    ...
    this.genderSelectionDropdownLocator = this.page.locator(
      '(//*[@class="_dropdown_1ew5s_1"])[2]'
    )
    ...
    this.contractTypeSelectionDropdownLocator = this.page.locator(
      '(//*[@class="_dropdown_1ew5s_1"])[3]'
    )
    this.clxTesting = page.getByRole('button', { name: 'Clusterix testing' })
    ...
    this.recruiterSelectionDropdownLocator = this.page.locator(
      '(//*[@class="_dropdown_1ew5s_1"])[4]'
    )
    this.wahyRecruiterOptionLocator = page.getByRole('button', {
      name: 'Wahyuu',
    })
    ...
    this.roleITOptionLocator = page.getByRole('button', { name: 'Moscow' })
    ...
    this.departmentSelectionDropdownLocator = this.page.locator(
      '(//*[@class="_dropdown_1ew5s_1"])[7]'
    )
    this.hrDepartmentOptionLocator = page.getByRole('button', {
      name: 'Moscow',
    })


    //ContractStatus Filter Locators
    this.contractStatusFilterCategory = page.locator('')
    this.contractStatusDropdown = page.locator(
      '(//*[@class="_button__name_l7kvi_19"])[14]'
    )
    this.contractStatusIn2MonthsOption = page.locator(
      '(//*[@class="_listItem__name_cx3fq_23"])[3]'
    )
    this.contractStatusAllOption = page.locator(
    ...
    this.datePickerInput = this.page.locator(
      '(//*[@class="_field__inputWrapper_dlunb_52"])[20]'
    )
    this.selectToday = page.locator('(//*[@class="_button__name_61s7l_25"])[5]')
  ...
  async searchAndVerifyEmployee(searchValue: string) {
    await this.fullTableLocator.waitFor({ state: 'visible', timeout: 50000 })
  ...
  async slecetGenderFilter() {
    ...
    await this.genderSelectionDropdownLocator.waitFor({
      state: 'visible',
      timeout: 3000,
    })
    await this.genderSelectionDropdownLocator.click()
    await this.page.waitForTimeout(1000)
    await this.maleGenderOptionLocator.click()
  }


  async slecetLocationFilter() {
    await this.filterToggleButtonLocator.waitFor({
      state: 'visible',
      timeout: 3000,
    })
    ...
    await this.DropdownLocator.waitFor({ state: 'visible', timeout: 3000 })
  ...
  async applyFilter(filters: { [key: string]: string }) {
    await this.fullTableLocator.waitFor({ state: 'visible', timeout: 120000 }) // Ensure table is loaded
    ...
    for (const [filterType, filterValue] of Object.entries(filters)) {
      ...
      await this.page.waitForTimeout(1000)
      const filterOptionLocator = this.getFilterOptionLocator(filterValue)
      ...
      await this.filterToggleButtonLocator.click() // Close dropdown
      appliedFilterCount++ // Increase filter count
    }
  ...
  async verifyAppliedFilter(filterType: string, expectedValue: string) {
    ...
    let appliedFilterLocator: Locator | null = null
    ...
    switch (filterType) {
      ...
      case 'office':
        appliedFilterLocator = this.searchLocationLocator
        await appliedFilterLocator.waitFor({ state: 'visible', timeout: 10000 })
        await expect(appliedFilterLocator).toHaveText(expectedValue)
        await this.ClearAllFilters()
        await this.refreshFilter()
        break
      ...
      case 'contractType':
        appliedFilterLocator = this.searchContractTypeLocator
        await appliedFilterLocator.waitFor({ state: 'visible', timeout: 10000 })
        await expect(appliedFilterLocator).toHaveText(expectedValue)
        await this.ClearAllFilters()
        await this.refreshFilter()
      ...
      case 'department':
        appliedFilterLocator = this.searchDepartmentLocator
        await appliedFilterLocator.waitFor({ state: 'visible', timeout: 10000 })
        await expect(appliedFilterLocator).toHaveText(expectedValue)
        await this.ClearAllFilters()
        await this.refreshFilter()
  ...
  async verifyClearAllFilters() {
    ...
    await this.slecetLocationFilter()
  ...
  async verifyFilterWhenNoMatcheFound() {
    await this.fullTableLocator.waitFor({ state: 'visible', timeout: 90000 })
    await this.slecetLocationFilter()
  ...
  async applyContractStatusFilter() {
    ...
    // ✅ Check if contractStatusDropdown is already visible
    const isDropdownOpen = await this.DropdownLocator.isVisible()


    if (!isDropdownOpen) {
      // 🔹 If not open, click contractStatusFilterCategory to open it
      await this.contractStatusFilterCategory.waitFor({
        state: 'visible',
        timeout: 3000,
      })
      await this.contractStatusFilterCategory.click()
    }
    await this.contractStatusDropdown.waitFor({
      state: 'visible',
      timeout: 3000,
    })
    await this.contractStatusDropdown.click({ force: true })


    await this.contractStatusAllOption.waitFor({
      state: 'visible',
      timeout: 3000,
    })
    await this.contractStatusAllOption.click({ force: true })
    await this.collapseButtonLocator.click()
  ...
  async applyContractStatusFilters() {
    await this.fullTableLocator.waitFor({ state: 'visible', timeout: 90000 })
    await this.filterToggleButtonLocator.click()
    await this.resetAllFilters.click()
  ...
  async fillDetailsAndSave() {
    ...
    await this.firstNameInput.waitFor({ state: 'visible', timeout: 900000 })
    await this.firstNameInput.fill(firstName)
    await this.lastNameInput.waitFor({ state: 'visible', timeout: 900000 })
    ...
    // Return the generated data so the test can verify
  ...
  async deleteAnEmployee(employeeName: string) {
    ...
    await this.deleteButtonLocator.waitFor({ state: 'visible', timeout: 10000 })
    await this.deleteButtonLocator.click()
    await this.deleteToastMessage.waitFor({ state: 'visible', timeout: 10000 })
  ...
  generateRandomString(): string {
    ...
    // Randomly choose either length 5 or 6
