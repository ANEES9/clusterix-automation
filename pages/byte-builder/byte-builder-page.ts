...
export class BytebuilderPage {
  ...
  constructor(page: Page, locale: string) {
    ...
    this.newSchemaButton = page.getByText(this.translations.schemas_page.new_schema)
    this.schemaItemsText = page.getByText(this.translations.create_schema.side_panel.schema_items)
    this.newSchemaTitle = page.getByText(this.translations.create_schema.title_placeholder)
    this.blankEditorText = page.getByText(this.translations.create_schema.create_tab.drag_items)
    this.textStringButton = page.getByRole('button', { name: 'Text/String' })
    this.fieldNameInput = page.getByLabel(this.translations.create_schema.create_tab.field_name)
    this.markAsRequiredCheckbox = page.getByText(this.translations.create_schema.create_tab.mark_required)
    this.minLengthInput = page.getByLabel(this.translations.create_schema.create_tab.minimum_length)
    this.maxLengthInput = page.getByLabel(this.translations.create_schema.create_tab.maximum_length)
    this.minValueInput = page.getByLabel(this.translations.create_schema.create_tab.minimum_value)
    this.maxValueInput = page.getByLabel(this.translations.create_schema.create_tab.maximum_value)
    ...
    this.saveButton = page.getByRole('button', { name: this.translations.common.save })
    ...
    this.schemaSearchInput = page.getByPlaceholder(this.translations.schemas_page.search_placeholder)
    this.schemaTable = page.locator('.SmartTable-module_tableWrapper__O2vT5')
    this.openSchemaTitleInputField = page.locator('[id="headlessui-control-\\:ri\\:"]')
    this.schemaActionLocator = page.locator(`tr:has-text("${schemaTestData.schemaTitle}") div.DRwDheUAGb0GrsZrlegi.BaseCellWrapper-module_wrapper__XCaQu svg`)
    this.schemaHomeButton = page.getByRole('button', { name: 'Schemas' })
    this.schemasHomePageTitle = page.getByText(this.translations.pages.schemas).nth(2)
    this.importSchemaButton = page.getByRole('button', { name: this.translations.schemas_page.import })
    this.schemaCancelButton = page.getByRole('button', { name: this.translations.common.cancel })
    this.schemaCloseButton = page.locator('.styles-module_headerCloseButton__x2ELS')
    this.schemaCancelConfimModal = page.locator('#confirm-modal')
    this.ConfirmationModalCancel = page.locator('#confirm-modal').getByRole('button', { name: this.translations.common.cancel })
    this.ConfirmationModalConfirm = page.getByRole('button', { name: this.translations.common.confirm })
    this.schemaCancelConfimModalTitle = page.getByText(this.translations.create_schema.delete.title)
    this.schemaCancelConfimModalMessage = page.getByText(this.translations.create_schema.delete.message)
  ...
  async verifySchemaPublished() {
    await Allure.step('Verify schema is published', async () => {
      await Allure.step('Check if the "Schema published" message is visible', async () => {
        const schemaPublishedMessage = this.translations.redux.success.publish_schema

        await this.page.waitForSelector(`text=${schemaPublishedMessage}`, { state: 'visible' })
        await expect(this.page.getByText(schemaPublishedMessage)).toBeVisible()
      })
    })
  }
  ...
  async assertSchemaPresence(title: string, shouldExist: boolean) {
    await Allure.step(`Verify schema "${title}" ${shouldExist ? 'is present' : 'is not present'} in the table`, async () => {
  ...
  async verifyEditSchema() {
    await Allure.step('Verify the saved schema items for Text and Number fields', async () => {
      await Allure.step(`Verify the text field name "${schemaTestData.fieldName}" is visible`, async () => {
        await expect(this.page.getByText(schemaTestData.fieldName)).toBeVisible()
      })
      await Allure.step(`Verify the number field name "${schemaTestData.numberFieldName}" is visible`, async () => {
        await expect(this.page.getByText(schemaTestData.numberFieldName, { exact: true })).toBeVisible()
      })
  ...
  async verifyNewlyCreatedSchema() {
    await Allure.step('Verify the opened schema and its content', async () => {
      await Allure.step('Verify the schema title is visible', async () => {
        await expect(this.page.locator('#root')).toContainText(schemaTestData.schemaTitle)
      })

      await Allure.step('Verify "Welcome to your schema" is visible', async () => {
        await expect(this.page.getByText(this.translations.schema_page.blank_page.welcome)).toBeVisible()
      })

      await Allure.step('Verify "Import your data or add it" is visible', async () => {
        await expect(this.page.getByText(this.translations.schema_page.blank_page.import_your_data)).toBeVisible()
      })

      await Allure.step('Verify "Add data manually" button is visible', async () => {
        await expect(this.page.getByRole('button', { name: this.translations.schema_page.blank_page.add_manually })).toBeVisible()
      })

      await Allure.step('Verify "Import from File" button is visible', async () => {
        await expect(this.page.getByRole('button', { name: this.translations.schema_page.blank_page.import_file })).toBeVisible()
      })
  ...
  async editSchemaTitle(newTitle: string) {
    await Allure.step(`Edit schema title to: ${newTitle}`, async () => {
      await this.page.locator('span').filter({ hasText: schemaTestData.schemaTitle }).click()
  ...
  async verifySchemaTitle(expectedTitle: string) {
    await Allure.step(`Verify schema title is updated to: ${expectedTitle}`, async () => {
      await expect(this.page.locator('span').filter({ hasText: expectedTitle })).toBeVisible()
    })
  }
  ...
  async verifySchemaCancelModal() {
    await Allure.step('Verify schema cancel confirmation modal text and buttons', async () => {
  ...
  async clickCancel() {
    await Allure.step('Click on the Cancel button in the schema cancel confirmation modal', async () => {
      await this.ConfirmationModalCancel.click()
    })
  }

  async clickConfirm() {
    await Allure.step('Click on the Confirm button in the schema cancel confirmation modal', async () => {
      await this.ConfirmationModalConfirm.click()
    })
  }

  async verifyModalClosedAfterCancel() {
    await Allure.step('Verify the schema cancel confirmation modal is closed and schema items list is visible after clicking Cancel', async () => {
  ...
  async verifyModalClosedAfterConfirm() {
    await Allure.step('Verify the schema cancel confirmation modal is closed and home page elements are visible after clicking Confirm', async () => {
      await expect(this.schemaCancelConfimModal).not.toBeVisible()
      await expect(this.schemasHomePageTitle).toBeVisible()
      await expect(this.schemaSearchInput).toBeVisible()
      await expect(this.importSchemaButton).toBeVisible()
  ...
  async deleteSchema(action: 'confirm' | 'cancel') {
    await Allure.step(`Delete schema: ${schemaTestData.schemaTitle}`, async () => {
      ...
      if (action === 'confirm') {
        await this.page.getByRole('button', { name: confirmButtonText }).click()
      } else if (action === 'cancel') {
        await this.page.getByRole('button', { name: cancelButtonText }).click()
      }
    })
  }
  ...
  async verifySchemaDeletionMessage(shouldExist: boolean) {
    await Allure.step('Verify schema deletion message', async () => {
      const deletionMessageLocator = this.page.locator(`text=${this.translations.redux.success.delete_schema}`)
