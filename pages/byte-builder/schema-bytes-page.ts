...
export class SchemaBytesPage {
  ...
  constructor(page: Page, locale: string) {
    ...
    this.addDataManuallyButton = page.getByRole('button', {
      name: this.translations.schema_page.blank_page.add_manually,
    })
    this.addSchemaItemButton = page.getByRole('button', {
      name: this.translations.schema_page.add_data,
    })
    this.addSchemaItemPageTitle = page.getByText(
      this.translations.schema_page.edit_panel.add_data,
      { exact: true }
    )
    this.cancelSchemaItemButton = page.getByRole('button', {
    ...
    this.requiredText = page.getByText(
      this.translations.create_schema.create_tab.required
    )
    this.textLengthError = page.getByText(
      this.translations.validations.between_length
        .replace('{{min}}', schemaTestData.minLength)
        .replace('{{max}}', schemaTestData.maxLength)
    )
    this.numberRangeError = page.getByText(
      this.translations.validations.between_value
        .replace('{{min}}', schemaTestData.minValue)
        .replace('{{max}}', schemaTestData.maxValue)
    )
    this.saveByteSuccessMessage = page.getByText(
      this.translations.redux.success.add_byte
    )
    this.searchByte = page.getByRole('textbox', {
      name: this.translations.schema_page.search_data_placeholder,
    })
    this.bytesTable = page.locator('.SmartTable-module_tableWrapper__O2vT5')
    this.bytesTableSettingIcon = page
      .locator('button.Bar-module_part__CpFuS')
      .nth(1)
    this.bytesTableSelectButton = page
      .locator('button.Bar-module_part__CpFuS')
      .first()
    this.schemaEditButton = page.getByRole('button', {
      name: this.translations.schema_page.edit_schema,
    })
    this.schemaDeleteButton = page.getByRole('button', {
      name: this.translations.schema_page.delete_schema.label,
    })
    this.importFileButton = page.getByRole('button', {
      name: this.translations.schema_page.import_from_file,
    })
    this.exportAllDataButton = page.getByRole('button', {
      name: this.translations.schema_page.export_all,
    })
    this.selectAllButton = page.getByText(
      this.translations.additional.select_all
    )
    this.exportSelectionButton = page.getByText(
      this.translations.additional.export_selected
    )
    this.deleteSelectionButton = page.getByText(
      this.translations.additional.delete_selected
    )
    ...
    this.updateSchemaByteModalTitle = page
      .getByText(this.translations.schema_page.edit_panel.update_data)
      .first()
    this.updateSchemaByteButton = page.getByRole('button', {
      name: this.translations.schema_page.edit_panel.update_data,
    })
  ...
  async verifyPageElements() {
    await Allure.step(
      'Verify Schema item creation page elements are visible',
      async () => {
        await Allure.step(
          'Verify Add Schema Item Page Title is visible',
          async () => {
  ...
  async verifyTextFieldLengthError(shouldBeVisible: boolean) {
    await Allure.step(
      `Verify text length error should ${shouldBeVisible ? 'appear' : 'not appear'}`,
      async () => {
        if (shouldBeVisible) {
  ...
  async verifyNumberValueError(shouldBeVisible: boolean) {
    await Allure.step(
      `Verify number value error ${shouldBeVisible ? 'is' : 'is not'} visible`,
      async () => {
        if (shouldBeVisible) {
  ...
  async searchSchemaBytes(isValid: boolean) {
    await Allure.step(
      `Search schema using ${isValid ? 'valid' : 'invalid'} data`,
      async () => {
  ...
  async assertSearchResults(isValid: boolean) {
    await Allure.step(
      `Verify search results for ${isValid ? 'valid' : 'invalid'} data`,
      async () => {
        if (isValid) {
  ...
  async verifySelectionActionsVisible() {
    await Allure.step(
      'Verify "Export Selection" button is visible',
      async () => {
    ...
    await Allure.step(
      'Verify "Delete Selection" button is visible',
      async () => {
  ...
  async editSchemaByte() {
    await Allure.step(
      'Click on the first schema byte edit button',
      async () => {
  ...
  async verifyByteEditModal() {
    await Allure.step(
      'Verify that the "Update Data" modal title is visible',
      async () => {
    ...
    await Allure.step(
      'Verify that the "Update Data" button is visible',
      async () => {
