...
export class templateManagerPage {
  ...
  constructor(page: Page) {
    ...
    this.selectPageSizeDropdown = this.page.getByRole('button', { name: 'A4 (8.3x11.7 inch)' })
    // Initialize locators for various buttons, fields, and sections
    this.templateManagerLink = this.page.getByRole('link', { name: 'Template Manager Easily' })
    this.newTemplateButton = this.page.getByRole('button', { name: 'New Template' })
    this.templateSizeButtonA3 = this.page.locator('div').filter({ hasText: /^A3 \(11\.7x16\.5 inch\)$/ })
    this.templateSizeButtonA4 = this.page.locator('div').filter({ hasText: /^A4 \(8\.3x11\.7 inch\)$/ })
    this.templateSizeButtonA5 = this.page.locator('div').filter({ hasText: /^A5 \(5\.8x8\.3 inch\)$/ })
    this.editor = this.page.locator('#root section div').first()
    this.createTemplate = this.page.getByRole('button', { name: 'Create Template' })
    ...
    this.templateDescriptionField = this.page.getByPlaceholder('Add description...')
    this.templateTagField = this.page.getByPlaceholder('Enter tag and press Enter')
    this.tagSelection = this.page.locator('.CSy9VRnimbLS3JrnWJq7 > .ca-max-h-64 > div:nth-child(6)')
