...
export class ProjectTypesPage {
  ...
  constructor(page: Page, locale: string) {
    ...
    this.newProjectTypeButton = page.getByRole('button', {name: this.translations.projectTypes.newProjectType,})
    this.projectTypeModal = page.locator('#project-type-modal-content')
    this.titleInput = page.getByPlaceholder(this.translations.projectTypes.typeModal.titlePlaceholder)
    this.colorList = page.locator('span[class*="ColorItem-module_color"]')
    this.descriptionInput = page.getByPlaceholder(this.translations.projectTypes.typeModal.descriptionPlaceholder)
    this.workFlowCategory = page.getByPlaceholder(this.translations.projectTypes.typeModal.category)
    this.workFlowDropdown = page.locator('.ca-max-h-64')
    this.selectWorkflowDropdown = page.getByRole('button', { name: 'Agile Flow' })                                                        
    this.manageParticipantsButton = page.getByRole('button', {name: this.translations.projectTypes.typeModal.manageParticipants})
    this.manageParticipantsModal = page.locator('.YorUFdmGYZRv4Eqdz23e')
    this.searchParticipantsInput = page.getByPlaceholder(this.translations.projectTypes.typeModal.searchResponsibles)
    ...
    this.createButton = page.getByText(this.translations.projectTypes.typeModal.save)
    this.projectTypeHeader = page.getByRole('button', {name: this.translations.main["project-types"]})
    ...
    this.saveProjectType =  page.getByText(this.translations.projectTypes.typeModal.saveEdit)
    this.deleteIcon = page.locator('div.ca-flex svg[viewBox="0 0 14 14"]').nth(3)
    this.deleteButton = page.getByRole('button', { name: this.translations.projectTypes.deleteProjectTypeNoProjects })
    this.projectTypeDeleteModal = page.getByText(this.translations.projectTypes.deletePromptHintNoProjects)
    ...
    this.searchByProjectFilter = page.getByPlaceholder(this.translations.projectTypes.search)
  ...
  async clickManageParticipantsButton() {   
    await Allure.step('Click on manage participants button and add participants', async () =>  {                 
  ...
  async verifyProjectTypeVisible() {
    ...
    await expect(this.projectTypeTitle).toBeVisible()
      ...
      console.log(`New Project Type: ${projectTypeTestData[0].title} is created successfully`);
    })
  }
  ...
  async deleteProjectType() {
   ...
   async validateDeleteProjectType() {
    await Allure.step('Validate that selected project type is deleted successfully' , async () => {
      await expect(this.updatedProjectTitle.locator(`:has-text("${projectTypeTestData[1].title}")`)).toHaveCount(0);
  ...
  async validatingSearchForProjectTypeFilter() {
    await Allure.step('Validating the search for contractor filter in contractor allocation', async () => {
      ...
      const projectTypeLocator = this.page.locator('SmartTable-module_tableWrapper__O2vT5')
      const count = await projectTypeLocator.count();
  await Allure.step(`Total Visible Project Type: ${count}`, async () => {
  })
