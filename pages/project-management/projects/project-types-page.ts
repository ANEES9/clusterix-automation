...
export class ProjectTypesPage {
  ...
  constructor(page: Page, locale: string) {
    ...
    this.selectParticipant = page
      ...
      .getByText('Mohamed AlFarsi')
    this.closeManageParticipantsModal = page
      .locator('#undefined')
      .getByRole('button')
      .first()
  ...
  async goto(baseURL: string | undefined) {
    await Allure.step(
      'Navigate to project management project type URL',
      async () => {
    ...
    await Allure.step(
      ...
      async () => {
        ...
        await this.searchParticipantsInput.fill(
          projectTypeTestData[0].participants[0]
        )
  ...
  async rdtoggleProjectType() {
    await Allure.step(
      'Enable R&D toggle button and validate the enabling',
      async () => {
  ...
  async clickCreateButton() {
    await Allure.step(
      'Click on create button to creata a project type',
      async () => {
  ...
  async verifyProjectTypeVisible() {
    await Allure.step(
      'Validate the newly created project type is visible on the project type page',
      async () => {
  ...
  async clickOnEditButton() {
    await Allure.step(
      'Click on the edit button of a project type',
      async () => {
  ...
  async updatedProjectType() {
    await Allure.step(
      'Validate that updated details of project type is visible on the page',
      async () => {
        const updatedProjectTitle = this.page.locator(
          `span:has-text("${projectTypeTestData[1].title}")`
        )
        await expect(updatedProjectTitle).toBeVisible()
        const updatedProjectDescription = this.page.locator(
          `span:has-text("${projectTypeTestData[1].description}")`
        )
        await expect(updatedProjectDescription).toBeVisible()
        const projectTypeRow = this.page
