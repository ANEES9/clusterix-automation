...
export class OverviewPage {
  ...
  private searchIconForProject1: Locator
    ...
    constructor(page: Page, locale: string) {
        ...
        this.overviewButton = page.getByRole('button', { name: this.translations.main.sidebar.projects })
        this.tableProject = page.locator('.BaseCellWrapper-module_wrapper__XCaQu').first()
        this.allTypesFilter = page.getByPlaceholder(this.translations.main["All Types"])
        this.allTypesFilterDropdownList = page.locator('._list_dd6fb_14')
        this.firstDropdownOption = page.getByRole('button', { name: 'SFF' , exact: true })                                            
        this.sffHeader = page.locator('div.project-management-overview-title b')
        this.secondDropdownOption =  page.getByRole('button', { name: 'Agile Software Projects' })  
        this.viewButton = page.getByRole('button', { name: this.translations.main.overviewMode["list"] })
        this.tileViewButton = page.getByText(this.translations.main.overviewMode["tile"])
        this.tileViewCardHeader = page.locator('.project-management-overview-tilesview')                              
        this.viewButton2 = page.getByRole('button' , { name: this.translations.main.overviewMode["tile"] } )
        this.kanbanViewButton = page.getByText(this.translations.main.overviewMode["kanban"])
        this.kanbanViewCardHeader = page.locator('.project-management-overview-kanbanview')
        this.filteredProjectsHeader = page.getByText(this.translations.main["filtered_projects"])
        this.addProjectButton = page.getByText(this.translations.main["Add project"])
        this.addProjectModal = page.locator('._modal_lyue6_22')
        this.closeAddProjectModal = page.getByRole('button' , { name: this.translations.common.cancel})
        this.selectProjectTypePlaceholder = page.locator(`input[placeholder="${this.translations.main["Please select project type"]}"]`)
        this.selectProjectType =  page.getByRole('button', {name: 'SFF', exact: true})
        this.shortTitleInput = page.locator(`input[placeholder="${this.translations.main["Please enter short title"]}"]`)
        this.shortTitleInputCharacterCount = page.locator(
            'div:has(input[placeholder="' + this.translations.main['Please enter short title'] + '"]) .characters-counter-danger').first();
        this.projectTitleInput = page.locator(`input[placeholder="${this.translations.main["Please enter title"]}"]`)
        this.projectStart  = page.getByLabel(this.translations.main["Project start"])
        this.selectingStartDate = page.getByRole('button', { name: '1', exact: true }).first()
        ...
        this.createProjectButton = page.getByRole('button' , { name: this.translations.main["Create project"]})
        this.projectCreatedSuccessMessage = page.getByText(this.translations.main["projects created successfully"])
        this.projectTitleInputCharacterCount = page.locator('input[placeholder="' + this.translations.main["Please enter title"] + '"].characters-counter-danger').nth(1);
        this.searchIconForProject = page.locator('tbody tr:nth-child(1) td:nth-child(12) svg.ca-fill-theme')
        this.searchIconForProject1 = page.locator('.ca-fill-theme').first()
        this.filterButtonOverview = page.getByRole('button', { name: "Filters" })
        this.selectShowProjectsOption = page.getByRole('button', { name: this.translations.main.show_projects })
        this.hidden = page.getByText(this.translations.main["Hidden Projects"])
        this.projectListBody = page.getByRole('table').locator('div').filter({ hasText: 'Automation' }).nth(2)
        this.settingIcon = page.locator('.Bar-module_part__CpFuS').nth(2)
        this.makeGroupingButtonOn = page.getByRole('button', { name: this.translations.main["turn_on_grouping"] })
        this.makeGroupingButtonOff = page.getByRole('button', { name: this.translations.main["turn_off_grouping"] })
        this.projectTypeGrouping = page.locator('.project-management-overview-listview b')
...
async clickCreateProjectButton() {
    await Allure.step('Click on create project button', async () => {
        ...
        await expect(this.projectCreatedSuccessMessage).toBeVisible({timeout: 3000})
...
async fillProjectTitleInputCharaterCount() {
    await Allure.step('Fill project title input', async () => {
        await this.projectTitleInput.fill(addProjectData.lessThanMinProjectTitleInput)
        expect(this.projectTitleInputCharacterCount)
        await this.projectTitleInput.fill(addProjectData.moreThanMaxProjectTitleInput)
        expect(this.shortTitleInputCharacterCount)
        await this.projectTitleInput.fill(addProjectData.projectTitleInput)
    })
...
async searchIconForProjectVisible() {
    await Allure.step('Search icon for project is visible', async () => {
        await this.searchIconForProject1.waitFor({ state: 'visible', timeout: 10000 })
...
async navigateToOverview() {
    await Allure.step('Expand the side bar and navigate to ovevriew', async () => {
...
async validateHiddenFilter() {
    await Allure.step('Select hidden in the filter and validate that hidden project visible', async () => {
...
async makeGroupingOn() {
    await Allure.step('Make the grouping on and validate that all project type title is visible', async () => {
     ...
     await this.page.waitForSelector('.project-management-overview-listview b', { state: 'visible' })
