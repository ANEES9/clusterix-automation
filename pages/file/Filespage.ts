...
export class FilesPage {
  ...
  constructor(page: Page) {
    ...
    this.trashButton = this.page.locator('tr:nth-child(2) > td:nth-child(6) > .fTrbfOFwnQNNmUXd5tm3');
    this.sharedWithMeButton = this.page.getByRole('button', { name: 'Shared with me' });
   ...
   this.tagsSection = page.locator('div').filter({ hasText: /^TagsEnter tags$/ }).nth(1);
   this.tagInput = page.getByRole('textbox', { name: 'Enter tag and press enter' });
   this.tagByText = (tagName: string) => page.getByText(tagName);

   this.uploadButton = this.page.locator('div').filter({ hasText: /^Upload file$/ }).nth(1);
