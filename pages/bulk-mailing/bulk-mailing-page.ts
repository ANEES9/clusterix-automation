...
export class BulkMailingPage {
  ...
  constructor(page: Page) {
    ...
    this.bulkMailing = page.getByRole('link', { name: 'Bulk Mailing Send one-time' });
    ...
    this.dashboardButton = this.page.getByRole('button', { name: 'Dashboard' });
    ...
    this.importFromClusterix = this.page.locator('label').filter({ hasText: 'Import from ClusterixSelect a' });
    this.fileUpload = this.page.locator('label').filter({ hasText: 'Upload from FilesUpload' });
    this.copyPaste = this.page.locator('label').filter({ hasText: 'Copy & PasteManually paste' });
    ...
    this.sendNowButton = this.page.locator('label').filter({ hasText: 'Send nowSend your email right' });
    ...
    this.backToCampaignsButton = this.page.getByRole('button', { name: 'Back to campaigns' });
    this.excludeRecipientDropdown = this.page.getByLabel('Exclude recipient (optional)');
    this.saveAsDraftButton = this.page.getByText('Save as draft');
    this.addParticipantButton = this.page.locator('div').filter({ hasText: /^BackUntitled campaignSave as draftsend$/ }).getByRole('button').nth(1);
    this.closeAddParticipantModalButton = this.page.locator('#undefined').getByRole('button');
    this.searchParticipantField = this.page.getByPlaceholder('User Name');
    this.paricipantList = this.page.getByRole('heading', { name: 'Participant List' });
    this.cancelAddParticipantButton = this.page.locator('div').filter({ hasText: /^Cancel$/ }).first();
    this.saveAddParticipantButton = this.page.locator('div').filter({ hasText: /^Save$/ }).first();
    this.campaignDetailsSectionButton = this.page.locator('li').filter({ hasText: 'Details' });
    this.campaignSendTimeSectionButton = this.page.locator('li').filter({ hasText: 'Send Time' });
    this.selectTimeButton = this.page.getByRole('button', { name: 'Select a delivery date' });
    ...
    this.variablesSectionButton = this.page.locator('li').filter({ hasText: 'Variables' });
    this.connectToAudienceButton = this.page.locator('label').filter({ hasText: 'Connect to audience' }).first();
    this.connectToAudience2Button = this.page.locator('label').filter({ hasText: 'Connect to audience' }).nth(1);
    this.campaignSendButton = this.page.getByText('send', { exact: true });
    this.Toastmsg = this.page.locator('.Toastify__toast-container--bottom-right');
    this.campaignCreatedText = this.page.getByText('Campaign created successfully');
    this.campaignUpdatedText = this.page.getByText('Campaign updated successfully');
    this.notificationsButton = this.page.getByRole('button', { name: 'Notifications' });
    this.notificationCard = this.page.locator('div:nth-child(27) > .NotificationCardWithHeader_wrapper__368pZ > .NotificationCard_wrapper__1-myM');
    this.openFirstInCampaignPerformance = this.page.locator('.SmartTable-module_sticky__inZzK > .SmartTable-module_cellContentWrapper__ppYvz > .SmartTable-module_cellContent__fmjZa > .DKumEEqKiJl6DXQFDyR_').first();
    this.nameCampaignPerformance = this.page.getByRole('cell', { name: 'new', exact: true }).locator('div').nth(1);
    this.sentFirstCampaignPerformance = this.page.locator('td:nth-child(2) > .SmartTable-module_cellContentWrapper__ppYvz > .SmartTable-module_cellContent__fmjZa > .DKumEEqKiJl6DXQFDyR_').first();
    this.bounceFirstCampaignPerformance = this.page.locator('td:nth-child(3) > .SmartTable-module_cellContentWrapper__ppYvz > .SmartTable-module_cellContent__fmjZa > .DKumEEqKiJl6DXQFDyR_').first();
    this.sideBarButton = this.page.locator('div').filter({ hasText: /^DashboardCampaignsAudience© innoscripta AG 2025Imprint-Data Protection-T&C$/ }).getByRole('button').nth(3);
    this.campaignNameColumn = this.page.getByTitle('Campaign Name').locator('div').nth(1);
    ...
    this.participantsColumn = this.page.getByTitle('Participants').locator('div').nth(1);
    this.statusColumn = this.page.getByTitle('Status').locator('div').nth(1);
    this.searchByCampaign = this.page.getByPlaceholder('Search by campaign name...');
    ...
    this.userFilterDropdownSearchField = this.page.getByPlaceholder('Search users...');
    ...
    this.audiencFilterDropdownSearchField = this.page.getByPlaceholder('Search audiences...');
    ...
    this.completedAfterDatePicker = this.page.getByPlaceholder('Select a date...');
    this.completedBeforeDatePicker = this.page.getByPlaceholder('Select a date...');
