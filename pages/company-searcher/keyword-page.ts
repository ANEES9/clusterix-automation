...
export class KeywordPage {
    ...
    constructor(page: Page, locale: string) {
        ...
        this.sidebarButton = this.page.locator('._sideBarButton__name_1gqqn_243').nth(1);  
        this.searchInput = this.page.locator('.PartWrapper-module_partWrapper__I8EIP input[name="query"]');
        this.searchButton = this.page.locator('.PartWrapper-module_partWrapper__I8EIP button');
    ...
    async verifySearchInput() {
        await Allure.step('Verify the search input placeholder', async () => {
            await expect(this.searchInput).toHaveAttribute('placeholder', this.translations.search_placeholder);
        });
    }

    async verifyButtonVisible(p0: string) {
        await Allure.step('Verify the visibility of the search button', async () => {
    ...
    async performSearch(searchTerm: string) {
        await Allure.step(`Perform search using the term: "${searchTerm}"`, async () => {
    ...
    async navigatePages(companyPage: { clickNextPageButton: (arg0: string) => any; validatePageChange: (arg0: number) => any; }, startPage: number, pagesToNavigate: number): Promise<void> {
        await Allure.step(`Navigate through ${pagesToNavigate} pages starting from page ${startPage}`, async () => {
            ...
            for (let i = 0; i < pagesToNavigate; i++) {
                ...
                currentPageNumber = await companyPage.validatePageChange(expectedNextPage);
            }
        });
    }
