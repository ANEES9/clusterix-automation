...
const replyEmailId = 'bhat@innoscripta.com'
const replyEmailSubject = 'Testing purpose email via automation - Forward'
const replyEmailBody = `Testing purpose email via automation - Forward. Sent at: ${currentDate.toString()}`

const forwardEmailId = 'bhat@innoscripta.com'
const forwardEmailSubject = 'Testing purpose email via automation - Forward'
const forwardEmailBody = `Testing purpose email via automation - Forward. Sent at: ${currentDate.toString()}`

let accountId: string | null = null

test.describe('Email Basic Operations - Send New Email, Reply on Existing Email, Forward existing Email', () => {
  ...
  test('reply on existing email', async ({ page }) => {
    emailPage = new EmailPage(page)
    Allure.addSeverity('critical')
    Allure.addTag('smoke')
    Allure.addDescription(
    ...
    await Allure.step('Step 3: Verify and Fill To Address Field', async () => {
      await emailPage.verifyToAddressField()
      await emailPage.fillAndEnterToAddress(replyEmailId)
    })

    await Allure.step('Step 4: Verify and Fill Subject Field', async () => {
      await emailPage.verifySubjectField()
      await emailPage.fillAndEnterSubject(replyEmailSubject)
    })
    await page.waitForTimeout(2000)
    await Allure.step('Step 5: Verify and Fill Email Body', async () => {
      await emailPage.verifyBodyofEmail()
      await emailPage.clickOnBodyAndFill(replyEmailBody)
    })
  ...
  test('forward an existing email', async ({ page }) => {
    emailPage = new EmailPage(page)
    Allure.addSeverity('critical')
    Allure.addTag('smoke')
    Allure.addDescription(
    ...
    await Allure.step('Step 3: Verify and Fill To Address Field', async () => {
      await emailPage.verifyToAddressField()
      await emailPage.fillAndEnterToAddress(forwardEmailId)
    })

    await Allure.step('Step 4: Verify and Fill Subject Field', async () => {
      await emailPage.verifySubjectField()
      await emailPage.fillAndEnterSubject(forwardEmailSubject)
    })
    await page.waitForTimeout(2000)
    await Allure.step('Step 5: Verify and Fill Email Body', async () => {
      await emailPage.verifyBodyofEmail()
      await emailPage.clickOnBodyAndFill(forwardEmailBody)
    })
