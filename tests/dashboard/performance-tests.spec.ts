...
test.describe('Clusterix Performance Tests', () => {
    ...
    async function measurePerformance(
        ...
        try {
            const navButton = page.getByRole('button', { name: navButtonName, exact: true });
    ...
    const pages = [
        { name: 'Files', headerSelector: 'div.ca-whitespace-nowrap.ca-text-ellipsis.ca-overflow-hidden:has-text("Upload file")', testName: 'Navigate to Files' },
        { name: 'Office', headerSelector: 'h1:has-text("Cluster Office")', testName: 'Navigate to Office' },
        { name: 'PDF', headerSelector: 'h1:has-text("Cluster PDF")', testName: 'Navigate to PDF' },
        { name: 'HR', headerSelector: 'span:has-text("Dashboard")', testName: 'Navigate to HR' },
        { name: 'Project Management', headerSelector: 'b:has-text("All Projects")', testName: 'Navigate to Project Management' },
        { name: 'Task Management', headerSelector: 'h1:has-text("Dashboard")', testName: 'Navigate to Task Management' },
        { name: 'Time Tracking', headerSelector: 'span:has-text("Timer")', testName: 'Navigate to Time Tracking' },
        { name: 'Company Searcher', headerSelector: 'div:has-text("Company")', testName: 'Navigate to Company Searcher' },
        { name: 'Customers', headerSelector: 'h1:has-text("Dashboard")', testName: 'Navigate to Customers' },
        { name: 'No Code', headerSelector: 'h1:has-text("About No-Code")', testName: 'Navigate to No Code' },
        { name: 'Template Manager', headerSelector: 'h1:has-text("All Templates")', testName: 'Navigate to Template Manager' },
    ];

    for (const pageInfo of pages) {
        test(`Performance Test: ${pageInfo.name}`, async ({ page }) => {
            if (pageInfo.name === 'Company Searcher') {
                await measurePerformance(page, pageInfo.name, pageInfo.headerSelector, pageInfo.testName, async () => {
                    await closeCurrentlyActivePopup(page);
                });
            } else {
                await measurePerformance(page, pageInfo.name, pageInfo.headerSelector, pageInfo.testName);
            }
        });
    }
});
