...
test.describe('Bytebuilder > Schema Tests', () => {
  ...
  test('Verify schema cancel confirmation popup', async ({  }) => {
    Allure.addDescription('This test checks if the schema cancel confirmation modal appears when clicking the Cancel or Close button without saving.')
    Allure.addSeverity('normal')
    Allure.addTag('smoke')
    await Allure.step('Verify confirmation popup appears when user click on cancel and close button', async () => {
      await byteBuilderPage.clickNewSchemaButton()
      await byteBuilderPage.addSchemaTitle(schemaTestData.schemaTitle)
  ...
  test('Verify schema confirmation buttons functionality', async ({ page }) => {
    Allure.addDescription('This test ensures that the schema cancel confirmation modal closes when clicking Cancel and proceeds when Confirm is clicked.')
    Allure.addSeverity('normal')
    Allure.addTag('smoke')
    await Allure.step('Test the behavior of clicking Cancel and Confirm in the modal', async () => {
      await byteBuilderPage.clickNewSchemaButton()
      await byteBuilderPage.addSchemaTitle(schemaTestData.schemaTitle)
      await byteBuilderPage.clickSchemaCancelButton()
      await byteBuilderPage.clickCancel()
  ...
  test('Verify invalid schema search', async () => {
    ...
    await Allure.step('Search for an invalid schema and verify it is not present', async () => {
      ...
      await byteBuilderPage.assertSchemaPresence(schemaTestData.invalidSearchTitle, false)
    })
  })
  
  test('Verify schema edit', async () => {
    Allure.addDescription('Validates opening a saved schema and verifying its title and items.')
    Allure.addSeverity('critical')
    Allure.addTag('smoke')
    await Allure.step('Perform steps to search, open, and verify the schema details.', async () => {
      await byteBuilderPage.searchSchema(schemaTestData.schemaTitle)
  ...
  test('Verify schema open', async () => {
    Allure.addDescription('Validates opening a newly created schema from the Smart Table and verifying its title and items.')
    Allure.addSeverity('critical')
    Allure.addTag('smoke')
    await Allure.step('Perform steps to search, open newly created schema from Smart Table, and verify the schema details.', async () => {
      await byteBuilderPage.searchSchema(schemaTestData.schemaTitle)
  ...
  test('Open schema and edit title', async () => {
    await Allure.addDescription('This test searches for an existing schema, opens it, edits the title, and verifies the update.')
    Allure.addSeverity('normal')
    Allure.addTag('smoke')
  ...
  test('Verify schema home button functionality', async () => {
    await Allure.addDescription('This test opens a schema, clicks the schema button, and verifies navigation to the main schema page.')
    Allure.addSeverity('normal')
    Allure.addTag('smoke')
    await Allure.step('Open schema and navigate back to schema main page', async () => {
  ...
  test('Verify cancel schema deletion', async () => {
    Allure.addDescription('Validates that the schema deletion is cancelled and the schema is still present in the table.')
    Allure.addSeverity('normal')
    Allure.addTag('smoke')
    await Allure.step('Search for the schema and cancel deletion', async () => {
      await byteBuilderPage.searchSchema(schemaTestData.updatedSchemaTitle)
      await byteBuilderPage.deleteSchema('cancel')
      await byteBuilderPage.assertSchemaPresence(schemaTestData.updatedSchemaTitle, true)
      await byteBuilderPage.verifySchemaDeletionMessage(false)
    })
  })

  test('Verify confirm schema deletion', async () => {
    Allure.addDescription('Validates that confirming the schema deletion removes the schema from the table.')
    Allure.addSeverity('critical')
    Allure.addTag('smoke')
    await Allure.step('Search for the schema and confirm deletion', async () => {
      await byteBuilderPage.searchSchema(schemaTestData.updatedSchemaTitle)
      await byteBuilderPage.deleteSchema('confirm')
      await byteBuilderPage.assertSchemaPresence(schemaTestData.updatedSchemaTitle, false)
