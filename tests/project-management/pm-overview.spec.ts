...
test.describe('Project Management - Overview', () => {
  test.beforeEach(async ({ page, baseURL }) => {
    ...
    const titleLocator = page.locator(
      '.project-management-overview-title >> text=All Projects'
    )
  ...
  test('Get a project title and search for it', async ({ page }) => {
    ...
    await page.waitForSelector(
      '.project-management-overview .project-title-column span',
      { state: 'visible', timeout: 15000 }
    )
    const projectTitleLocator = page.locator(
      '.project-management-overview .project-title-column span'
    )
    ...
    const searchResult = page
      .locator('.project-title-column span')
  ...
  test("Verify navigation to a project's details from List View", async ({
    page,
  }) => {
  ...
  test("Verify navigation to a project's details from Tile View", async ({
    page,
  }) => {
  ...
  test("Verify navigation to a project's details from Kanban View", async ({
    page,
  }) => {
