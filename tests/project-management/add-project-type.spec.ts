...
test.describe('Clusterix Project Managament', () => {
  ...
  test('Add Project Type', async ({ page }) => {
    ...
    await page.locator('.ajRcq_m1OOoO3cbSsbzI.XSnRb6hz2HghB3Jl7GnF').scrollIntoViewIfNeeded()  // Scroll into view for project management
    ...
    const projectTypeNavigation = page.getByRole('button', { name: 'Project types' })
    await projectTypeNavigation.click()


    // Locate the button, click on it, and wait for the page to become idle
    const newProjectTypeButton = page.getByRole('button', { name: 'New project type' })
    ...
    const projectTypeTitle = await page.getByPlaceholder('Title of the project type')
    await projectTypeTitle.click()
    await page.getByPlaceholder('Title of the project type').fill(globalProjectTitle)
    console.log('Title for new project type is given')

    //Click on the color field and select the color
    const color = await page.locator('.Colors-module_colors__3SJ3e > span:nth-child(3)')
    await color.click()
    console.log('Color is selected')

    //Click on the description field and enter the description of the project type
    const descriptionProjectType = await page.getByPlaceholder('Description of the project')
    ...
    const manageParticipantsButton = await page.locator('button:has-text("Manage participants")')
    ...
    const searchParticipants = await page.locator('input[placeholder="Search responsible"]')
    ...
    const secondSuggestion = await page.locator('div.phOxFPhI39fmHfRR6Upk div').nth(1)  // This targets the second particioants in the list
    await secondSuggestion.click()
    console.log('Participants Selected')

    const closeButton = await page.locator('.styles-module_headerCloseButton__x2ELS').nth(1); // This targets the close button
    await closeButton.click()      // Click the close button

    // Locate the toggle button for R&D and click on it
    const toggleButton = await page.locator('.m4k0JlnIppWFWs69kSNm ._toggle_1x4ea_13')
    ...
    const projectTypeHeader = await page.getByRole('button', { name: 'Project types' })
    await projectTypeHeader.waitFor({ state: 'visible' })

    // Wait for the project title to appear in the project types list on main page
    const projectTitle = await page.locator(`span:has-text("${globalProjectTitle}")`)
    ...
    console.log(`New Project Type: ${createdProjectTitle} is created successfully`)
  ...
  test('Edit Project Type', async ({ page}) => {
         ...
         await page.waitForLoadState('networkidle');
          ...
          const projectRow =page.locator('td:nth-child(8) > .SmartTable-module_cellContentWrapper__ppYvz > .SmartTable-module_cellContent__fmjZa > .ca-gap-yellow').first()   //await page.locator(`text=${createdProjectTitle}`).first(); // This looks for the project title text
        ...
        // Retrieve the title field in the modal
         const projectTitleField = await page.locator('input[placeholder="Title of the project type"]') 
        ...
        const updatedcolor=await page.locator('span[style*="background-color: rgb(133, 10, 10);"]')
        await updatedcolor.click()
        console.log('Updated color is selected')
        
        const updateDescriptionProjectType=await page.getByPlaceholder('Description of the project')
         ...
         const manageParticipantsButton = await page.locator('button:has-text("Manage participants")')  // Targets the Maanage Participants Button
         await manageParticipantsButton.click()
        ...
        const closeButton = await page.locator('.styles-module_headerCloseButton__x2ELS').nth(1) // Alternatively use title or other attributes
        await closeButton.click()  // Click the close button
        ...
        await page.waitForSelector('span:has-text("New Type Automation")', { timeout: 10000 })
        ...
        const updatedDescriptionOnMainPage = await page.locator('text="Now I updated the description"') // Check for the updated description text
        await expect(updatedDescriptionOnMainPage).toBeVisible()
        console.log('Updated Description is visible on the main page')
        
        const projectTypeRow = await page.locator('td:has-text("New Type Automation")').locator('..')
  ...
  await projectTypeNavigation.click()
   ...
   const deleteButton = await page.locator('div.ca-flex svg[viewBox="0 0 14 14"]').nth(3)
  ...
  const clickdeleteButton = await page.locator('button:has-text("Delete Project type")')
