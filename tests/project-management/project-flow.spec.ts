...
test.describe('Clusterix Project Managament', () => {
  ...
  await page.locator('.ajRcq_m1OOoO3cbSsbzI.XSnRb6hz2HghB3Jl7GnF').scrollIntoViewIfNeeded()
    ...
    await page.waitForSelector('input[placeholder="Please select project type"]', { state: 'visible' })
    ...
    const shortTitleInput = page.locator('input[placeholder="Please enter short title"]')
   ...
   // Test case 1: Enter a short title with less than 3 characters
    ...
    await page.waitForTimeout(1000)
    const redMarkVisibleForValid = await page.locator('div:has(input[placeholder="Please enter short title"]) .characters-counter-danger').first().isVisible()
    ...
    const projectTitleInput = page.locator('input[placeholder="Please enter title"]')
    ...
    await projectTitleInput.fill("This is the new project I am creating to test the validation for Input field as well as for Automation test script")
    await page.waitForTimeout(1000)
    const redMarkVisibleForShort2 = await page.locator('input[placeholder="Please enter title"].characters-counter-danger').nth(1).isVisible()
    expect(redMarkVisibleForShort2).toBeFalsy()
    console.log('Validation passed for short project title with valid no. of characters')
    ...
    const calendar = page.locator('div[class*="DatePickerView-module_datePickerView"]').first()
  ...
  await page.waitForTimeout(500)
    ...
    const selectingDate = page.getByRole('button', { name: '9', exact: true }).nth(1)
    ...
    const confirmationMessage = await page.getByText("Project created successfully.").textContent()
    console.log(confirmationMessage)
    await expect(page.getByText("Project created successfully.")).toContainText('Project cr')
    await page.waitForTimeout(2000)
    })
  

  
  test('Validate the Title Input Field for Character counts', async ({ page }) => {
  ...
  await page.waitForTimeout(1000)
  const redMarkVisibleForShort = await page.locator('div:has(input[placeholder="Please enter short title"]) .characters-counter-danger').first().isVisible()
  ...
  await page.waitForTimeout(1000)
  const redMarkVisibleForLong = await page.locator('div:has(input[placeholder="Please enter short title"]) .characters-counter-danger').first().isVisible()
  ...
  await page.waitForTimeout(1000)
  const redMarkVisibleForShort1 = await page.locator('input[placeholder="Please enter title"].characters-counter-danger').nth(1).isVisible()
  expect(redMarkVisibleForShort1).toBeFalsy()
  console.log('Validation passed for short project title less than 50 characters')
  
  // Test case 2: Enter a long project title with more than 178 characters
  await projectTitleInput.fill("Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen Hello Sania Farheen")
  await page.waitForTimeout(1000)
  const redMarkVisibleForShort2 = await page.locator('input[placeholder="Please enter title"].characters-counter-danger').nth(1).isVisible()
  expect(redMarkVisibleForShort1).toBeFalsy()
  console.log('Validation passed for long project title less than 50 characters')
  
  // Test case 2: Enter a short project title with valid characters (between 50-178)
  await projectTitleInput.fill("This is the new project I am creating to test the validation for Input field as well as for Automation test script")
  await page.waitForTimeout(1000)
  const redMarkVisibleForShort3 = await page.locator('input[placeholder="Please enter title"].characters-counter-danger').nth(1).isVisible()
  expect(redMarkVisibleForShort2).toBeFalsy()
  console.log('Validation passed for project title with valid no. of characters')
  })
  ...
  test('Edit the project details', async ({ page }) => {               
    ...
    await page.waitForLoadState('networkidle')
       ...
       const searchIcon = page.locator('tbody tr:nth-child(1) td:nth-child(12) svg.ca-fill-theme')
       ...
       await projectTitle.fill("This is the new project created for automating the task using playwright")
       await page.waitForTimeout(500)
       await projectTitle.fill("")
       await projectTitle.fill("This is the new project I am creating to test the validation for Input field as well as for Automation test script and here I am adding few more words")
       await page.waitForTimeout(500)
       ...
       const navigateToOverview = page.locator('._sidebarSubMenuButton_16zcl_191 ._sidebarSubMenuButton__collapsedIcon_16zcl_195').first()
       await navigateToOverview.click()
       console.log("Navigated to Overview")
       await page.waitForTimeout(500)
    
       // Validate Updated Project Title
       const updatedProjectTitle = page.locator('span[class*="uYBBBNKxgHFoikuU642J"]').last()
       await updatedProjectTitle.waitFor({ state: 'visible' })
       await page.waitForTimeout(500)
       const updatedTitleText = await updatedProjectTitle.textContent()
       console.log(`The updated project title in the communication modal is: ${updatedTitleText}`)
    
       await page.waitForTimeout(500)
    
       if (updatedTitleText && updatedTitleText.includes("New")) {
          console.log("Project title updated successfully and is visible in the communication modal.") // Validation step to check the updated title 
       } else {
