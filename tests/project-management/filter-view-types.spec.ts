...
test.describe('Clusterix Project Managament', () => {
  ...
  test('All Projects Filter', async ({ page }) => {
    ...
    await page.waitForTimeout(1000)
    const isSFFChecked = page
      .getByRole('button', { name: 'SFF' })
      .getByLabel('')
      .isChecked() // Validate checkboxes are selected
    const sffHeader = await page.locator(
      'div.project-management-overview-title b'
    ) // This will locate the 'SFF' in the header
    ...
    const isAgileChecked = await page
      .getByRole('button', { name: 'Agile Software Projects' })
      .getByLabel('')
      .isChecked() // Validate checkboxes are selected
    const filteredProjectsHeader = await page.locator(
      'div.project-management-overview-title b'
    )
  ...
  test('Change the view types', async ({ page }) => {
    ...
    // Ensure page and elements are loaded
    await page.waitForLoadState('domcontentloaded')
    await page.waitForSelector('button.Bar-module_part__CpFuS:nth-of-type(2)', {
      state: 'visible',
    })
    ...
    await page.waitForSelector('button:has(span:has-text("Tile view"))', {
      state: 'visible',
    })
    ...
    const firstTileViewCardHeader = page
      .locator(
        'div.project-management-overview-tilesview-status-cards-card-header'
      )
      .first()
    ...
    const isFirstTileViewCardHeaderVisible =
      await firstTileViewCardHeader.isVisible()
    ...
    await page.waitForSelector('button:has(span:has-text("Kanban"))', {
      state: 'visible',
    })

    // Click on the Kanban button
    await page.click('button:has(span:has-text("Kanban"))')
    await page.waitForSelector('.project-management-overview-kanbanview', {
      state: 'visible',
    })

    const firstKanbanViewCardHeader = page.locator(
      '.project-management-overview-kanbanview'
    )

    // Verify that the first kanban view card header is visible
    const isFirstKanbanViewCardHeaderVisible =
      await firstKanbanViewCardHeader.isVisible()
    await page.waitForTimeout(6000)
