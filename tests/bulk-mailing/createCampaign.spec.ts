...
test.describe('Verify Bulk Mailing Functionalities', () => {
  ...
  test.beforeEach(async ({ page, baseURL }, testInfo) => {
    ...
    await page.waitForLoadState('domcontentloaded');
      ...
      await page.locator('div').filter({ hasText: /^Back$/ }).getByRole('textbox').fill('Demo');
  ...
  try {
    ...
    // Log error details if the assertion failed
    console.error("Assertion failed: 'Audience Created Text' element state did not meet expectations.", error);
}
try {
  // Await the assertion
  ...
  // Log success if the assertion passed
  console.log("Recieved Notification message: 'Audience Created Successfully'.");
} catch (error) {
  // Log error details if the assertion failed
  console.error("Assertion failed: 'Audience Created Notification' element state did not meet expectations.", error);
}
