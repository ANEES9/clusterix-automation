...
test.describe('No Code Overview Tests', () => {
  ...
  test.skip('Check Title Consistency Between Table Row and Modal Header', async ({
    page,
  }) => {
    // Locate the title text inside the first row's second cell
    const tableTitleLocator = page.locator(
      'tr:nth-child(1) td:nth-child(2) div.ca-flex.ca-items-center span'
    )
    ...
    const editButtonLocator = page.locator(
      'tr:nth-child(1) td button[aria-label="Edit"]'
    ) // Adjust if necessary
  ...
  //todo: fix
  test.skip('Open edit modal, hover over and edit page name', async ({
    page,
  }) => {
  ...
  test.skip('Drag and drop text field for pm workflow', async ({ page }) => {
    ...
    const source = page.locator(
      'div.DraggableTextItem-module_draggableTextItem__8lRSW'
    )
    const target = page.locator(
      'div.InputDropPlaceholder-module_sectionDropZoneWrapper__ur1HB'
    )
    ...
    const updatedTarget = page.locator(
      'div.InputDropPlaceholder-module_sectionDropZoneWrapper__ur1HB'
    )
