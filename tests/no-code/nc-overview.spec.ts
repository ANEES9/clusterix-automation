...
test.describe('No Code Overview Tests', () => {
    ...
    test('Create Accounting Workflow from overview tab', async ({ page }) => {
        const newNoCodeButton = page.locator('.ca-whitespace-nowrap.ca-text-ellipsis.hover\\:ca-cursor-pointer');
        await expect(newNoCodeButton).toBeVisible();
        await newNoCodeButton.click();
        const accountingBillsOption = page.locator('span:has-text("Accounting Bills")');
        await page.waitForSelector('span:has-text("Accounting Bills")', { state: 'visible' });
        ...
        await page.waitForSelector('span:has-text("Untitled No-Code")', { state: 'visible' });
