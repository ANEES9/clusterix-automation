...
test.describe('Dashboard contents', () => {
  ...
  //Test mo:case_id=827
  test.only('Verify User is able to see & apply Absence from Vacation Days', async ({
    page,
  }) => {
    await test.step('1- Navigate to My Absence Days and Verify vacation type and Days left', async () => {
      const vacationTypeText = await page
    ...
    await test.step('2- Click on the "+" symbol to add absence and verify', async () => {
      const addVacation = page
        .locator('div')
        .filter({
          hasText:
            /^My RequestsVacation Days 20240 Days left Sick DayHome Office Other$/,
        })
        .getByRole('button')
        .nth(1)
    ...
    await test.step('6- Click on the "Close" button on the confirmation dialogue', async () => {
      const vacationDaysClosePopup = page
        .locator('#undefined')
        .getByRole('button')
        .first()
