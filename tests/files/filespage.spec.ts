...
test.describe('File Reggression Suite', () => {
    ...
    // Validate navigation
          ...
          test('Download and Trash action', async ({ page }) => {
            ...
            await page.locator('td:nth-child(7) > .fTrbfOFwnQNNmUXd5tm3').first().click();
          ...
          test('Upload file button action', async ({ page }) => {
            ...
            // Upload the file
            const filePath ="C:/Users/User/Downloads/toms-basics-2024_completion_2024-12-26.pdf";

            await filesPage.uploadFile(filePath);
          
            // Verify the file upload (Optional: if there's a confirmation or visible file listing)
            const isFileUploaded = await filesPage.verifyFileUploaded('toms-basics-2024_completion_2024-12-26.pdf');
